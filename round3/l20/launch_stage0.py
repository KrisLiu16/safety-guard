"""Run source-label-free C1 Stage 0 distillation on an available L20."""
import argparse
import hashlib
import json
...
JOB = 'safety-guard-c1-stage0-20260923'
...
def verify_bundle():
    expected = (HERE / 'bundle_stage0.sha256').read_text().split()[0]
    digest = hashlib.sha256()
    with (HERE / 'bundle_stage0.tar').open('rb') as stream:
        for block in iter(lambda: stream.read(8 * 1024 * 1024), b''):
            digest.update(block)
    if digest.hexdigest() != expected:
        raise RuntimeError('Local bundle checksum mismatch')
    manifest = json.loads((HERE / 'bundle_stage0_manifest.json').read_text())
...
def main():
    ...
    parser.add_argument('--out', default=str(HERE / 'stage0_output'))
    ...
    run(kubeconfig, 'create', '-f', str(HERE / 'job_stage0.yaml'))
    ...
    run(kubeconfig, 'exec', '-i', pod, '-c', 'train', '--', 'sh', '-c', 'cat > /work/bundle.tar.part', input_file=HERE / 'bundle_stage0.tar')
    run(kubeconfig, 'exec', '-i', pod, '-c', 'train', '--', 'sh', '-c', 'cat > /work/bundle.sha256', input_file=HERE / 'bundle_stage0.sha256')

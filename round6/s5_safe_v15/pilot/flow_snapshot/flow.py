"""S5 safe half: one word per request, two long safe thinking passages; chat_completions or responses."""
...
def run(ctx: Context):
    ...
    if group.get("prompt_version") != PROMPT_VERSION or group.get("calls_per_term") != 1:
        raise ValueError("Unexpected S5 safe generation contract")
    ...
    ctx.evaluation.score({"reward": counts["complete"] / max(1, counts["terms"])},
                         detail="Words with two valid long thinking passages / total words; skips count as 0")

...
records whose label is unusable or excluded are skipped. Records:
...
def classify(positions, ends, labelled):
    ...
    for position, end in zip(positions, ends):
        ...
        if rank is not None:
            ...
            classes.append(CLASS_INDEX[rank])
...
def text_record(row, messages, source, role, labelled, encode, sample_id=None):
    ...
    positions, classes, undetermined = classify(positions, ends, labelled)
    if not positions:
...
def build(runA_rows, prefix_data, labels, encode):
...
def main():
    ...
    label_paths = {"runA": args.runA_labels, "runA_prompts": args.prompt_labels, "prefix_v2": args.prefix_labels,
                   "prefix_v2_user": args.prefix_user_labels}
    ...
    records, stats = build(read_jsonl(args.runA), prefix_data, labels, encode)
    args.output.mkdir(parents=True)
    for split, rows in records.items():
        with (args.output / f"records_{split}.jsonl").open("w", encoding="utf-8") as handle:
            for row in rows:
                handle.write(json.dumps(row, ensure_ascii=False) + "\n")
    report = {"version": "stage2-targets-v1", "class_index": {"safe": 0, "unsafe": 1, "controversial": 2},

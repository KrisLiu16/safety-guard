#!/usr/bin/env python3
"""Download judge-run archives, join judgments with the local answer key, and report agreement."""
...
def main():
    ...
    rows, table, controls = [], collections.defaultdict(collections.Counter), []
    ...
    for item_id, item in key.items():
        ...
        row = {**{k: item[k] for k in ("item_id", "sample_id", "word", "kind", "expected", "response_style", "cut_char")},
        ...
        rows.append(row)
        if item["kind"].startswith("control"):
            controls.append(row)
    ...
    summary = {"run_no": run.get("run_no"), "run_status": run.get("status"), "items": len(key),
               "judged": sum(1 for r in rows if r["judge_label"]), "failures": dict(failures),
               "by_kind": {kind: dict(counter) for kind, counter in table.items()},
               "agreement_with_expected": {kind: sum(1 for r in rows if r["kind"] == kind and r["judge_label"] == r["expected"])
                                           / max(1, sum(1 for r in rows if r["kind"] == kind and r["judge_label"])) for kind in table},
               "controls": [{k: r[k] for k in ("word", "judge_label", "judge_reason")} for r in controls]}

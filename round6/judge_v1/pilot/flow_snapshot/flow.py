"""Blind judge flow: one request per assistant text (full response or prefix)."""
...
from pipeline import PROMPT_VERSION, parse, request_body, response_text
    ...
    for position, item in enumerate(group["items"]):
        ...
        try:
            ...
            if response.get("stop_reason") not in ("end_turn", "stop_sequence"):
                ...
                errors, judgment = parse(response_text(response))

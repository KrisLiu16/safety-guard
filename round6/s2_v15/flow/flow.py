"""S2 hard-negative generation: one word per request, one prompt + two safe responses; chat_completions or responses."""
...
def run(ctx: Context):
    ...
    if group.get("prompt_version") != PROMPT_VERSION or group.get("calls_per_term") != 1:
        raise ValueError("Unexpected S2 generation contract")
    ...
    for position, seed in enumerate(group["terms"]):
        ...
        try:
            # Exactly one model request per word, no in-flow retry.
    ...
    ctx.evaluation.score({"reward": counts["complete"] / max(1, counts["terms"])},
                         detail="Words with a valid prompt and two valid responses / total words; skips count as 0")

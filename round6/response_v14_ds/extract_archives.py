#!/usr/bin/env python3
"""Download v14 (chat_completions / DeepSeek) attempt archives, re-validate every word locally and write JSONL + audit.
...
from chat_adapter import chat_text, finish_reason, parse_json  # noqa: E402
from pipeline import PROMPT_VERSION, slot_records, validate  # noqa: E402
...
def revalidate(value):
    ...
    if finish_reason(response) != "stop":
        return ["finish_reason:" + finish_reason(response)], []
    try:
...
def main():
            ...
            for aid, path in pool.map(download, attempts):
                with tarfile.open(path, "r:gz") as tar:
                    ...
                    for member in members:
                        ...
                        if any(e.startswith(("finish_reason", "request_or_parse")) for e in word_errors):
    ...
    summary = {
        ...
        "filtered_or_failed_words_by_group": dict(refusals_by_group),

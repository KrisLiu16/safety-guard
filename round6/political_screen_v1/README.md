...
## v1.3：本地排除表（T026 第 9 步第二次之后）

最终核对以后，剩下的误命中都是三遍模型结论一致的冷僻词或泛指词，再让模型判一遍也解决不了。所以：
- `redline_v1/list_fired_words.py` 列出每个词在哪些分层把文本提了档；
- 执行方人工核对那些把普通文本提档的词，写进本地排除表；
- `apply_policy.py --exclude-words` 读这张表。排除表和核对表都含词，不提交。

另外三条改动：
- 两个汉字、带 ambiguous 的词一律不匹配；
- 领导人词只有两遍复查都判负面才算负面，否则按变体；
- 初筛和最终核对跨组（事件、人物、组织和领导人之间）的词，只在政治文本里生效。

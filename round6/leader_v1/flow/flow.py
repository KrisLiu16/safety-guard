"""Leader-sensitivity data (T031): one seed per request, one conversation ending in the assistant thinking."""
...
def run(ctx: Context):
    ...
    if group.get("prompt_version") != PROMPT_VERSION or group.get("calls_per_term") != 1:
        raise ValueError("Unexpected leader generation contract")
    ...
    ctx.evaluation.score({"reward": counts["complete"] / max(1, counts["terms"])},
                         detail="Seeds with a valid conversation / total seeds; skips count as 0")

"""Blind single-text safety judge v2: identical policy prompt to v1, two wire APIs.

v2 only adds OpenAI chat_completions (e.g. DeepSeek via tokenhub, a reasoning model: only
message.content is parsed, reasoning_content is ignored) next to Anthropic Messages.
...
PROMPT_VERSION = "guard-judge-single-v2"
...
def request_body(model_name, item, wire_api="anthropic_messages"):
    ...
    if wire_api == "chat_completions":
        # Reasoning model: leave room for the hidden reasoning before the short JSON answer.

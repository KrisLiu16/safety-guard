...
所以改为在生成和重判之前，直接对词本身判一次。一次请求判 200 个词，只问一个问题：是不是针对国家领导人的贬损说法，回答 yes / no / unsure。只判词，不生成任何内容。用 luna 判，因为 DeepSeek 会拦截政治类词。
...
| `test_word_screen_cpu.py` | 6 项 CPU 单测 |

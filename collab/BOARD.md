...
| [T018](assignments/T018-stage1-train-eval.md) | 第一阶段训练读出头（risk / full 两个变体）并与现在的头对比 | 执行方 | 等 T017 完成和用户确认开始训练 | T017 |
...
1. **T018 是否开始训练**：冻结主干，只重训助手侧读出头（`risk`：只训 512→3 风险层，可直接替换权重；`full`：投影加风险层），用 prefix_v2 训练集加 Run A 混合训练，几分钟完成，之后自动评测。T017 缓存完成后才能开始。

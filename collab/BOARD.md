...
| [T030](assignments/T030-stage2-v2-alert-leader.md) | 第二阶段 v2：警觉档（软目标）、政治组合、领导人数据、普通用法数据、英文红线数据 | 执行方（兼设计） | 训完已评测（第 1 轮）：英文红线大幅恢复（按我们口径重标，提问召回 41.6→77.5%、AUC 0.879），领导人抽查 64/64 截、67/67 放；但中文红线回答侧流式 AUC 降 0.023–0.030、普通误报 0.7→1.4%、正常问题扫描 8/11（标准 ≤3），未全部通过 | T029 ✓、T031 ✓、T032 ✓ |
| [T031](assignments/T031-leader-data-v1.md) | 领导人敏感度数据（中英）+ 普通用法数据（v1.2 地名等，v1.3 按误截扫描补） | 执行方（兼设计） | 完成：合计 6,720 条（领导人 2,840、普通用法 1,465 + 2,415），usable 6,712，提问侧 5,223 条 | — |
| [T032](assignments/T032-english-redline-v1.md) | 英文红线数据：Aegis 2.0 train 提问 4,727 + 回答 2,921，用红线裁判重标 | 执行方（兼设计） | 完成：提问 usable 4,721、回答 2,903，已进 v2 / v3 | — |
| [T033](assignments/T033-distill-4b-v3.md) | 第二阶段 v3：每个角色加一个通用头，用 Qwen3Guard-Stream-4B 逐 token 蒸馏（KL 0.5 + 类别 CE 0.25），红线头照 v2 训练 | 执行方（兼设计） | 完成：红线头中文 AUC 回到 v1 的 0.02 以内，扫描 3/6，领导人 64/64、0/67；通用头回答侧 F1 75.1（Qwen3Guard 78.3）。v3.1 修正提问侧通用头（读点改为 `<|im_end|>`），提问 F1 72.3→77.2。本轮候选 = v3 + v3.1。未达标：思考侧扫描 6（≤3）、普通误报 1.36% | T030 数据 ✓ |
| [T034](assignments/T034-serving-lossless-speedup.md) | 推理引擎提速，输出与 v6 逐位相同（v7 主机路径，v8 Triton 融合） | 执行方（兼设计） | 完成：v8 与 v6 最大差 0；P95 ≤ 20 ms 的会话数 208 → 352（+69%），15.2K token/s | — |
| [T035](assignments/T035-stage2-v4.md) | 第二阶段 v4：提问侧只在结尾学老师，补 normal3 普通对话数据 | 执行方（兼设计） | 完成：普通误报 1.36% → 0.34% ✓；回答侧扫描 5（≤ 3 ✗）；提问 F1 77.2 → 79.4。官方 thinking 集误报 2.5% → 6.2% | T033 ✓ |
| [T036](assignments/T036-train-eval-speed.md) | 训练和评测提速（剖析、批量 loss、分片评测、固定 Triton 配置） | 执行方（兼设计） | 完成：训练每步 0.82 → 0.31 秒；评测回答侧 2,023 → 616 秒，固定配置后各次逐行相同 | — |
| [T037](assignments/T037-stage2-v5.md) | 第二阶段 v5：英文长思考、政治同形词普通回答、通用头在基准 fit 半学 4B（held 半报告），警觉档 0.3 | 执行方（兼设计） | 完成：官方 thinking 集误报 6.2% → 2.1% ✓，扫描 1 / 0 ✓，回答普通误报 0.2% ✓；held 半通用头 F1 提问 85.4（0.6B 85.5）、回答 77.4（0.6B 77.2）；官方 thinking 召回 63% → 55% | T035 ✓、T036 ✓ |

...
1. 提问侧怎么分档（提问也按同一份红线清单判，但档位没定）。设计方的建议：
   - 提问本身写出红线违规内容（例如自己写出对领导人的侮辱，或贴出一段谣言）：风险；
   - 请求生成红线内容（例如“帮我写一篇宣扬恐怖主义的文章”）：风险；
   - 引用违规原话，或问违规词、违规口号是什么意思：有争议；
   - 只问红线话题（新闻、历史、科普），或请求辟谣：安全。

   定了以后，设计方给裁判加提问侧模式，在 T021 之后标注提问，再在 T023 的缓存上重训用户头。

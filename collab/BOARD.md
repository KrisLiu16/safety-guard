...
| [T014](assignments/T014-probe-transfer.md) | 探针迁移检验：同一探针在 prefix_v2 和官方 thinking 集上是否仍好于头（排除只学到 luna 风格的可能） | 执行方 | 待执行 | T004 ✓ |
...
| T008 | 第六轮训练。按 T004 结果，第一阶段先只重训读出头（投影层和风险头，主干冻结，稠密前缀监督），成本低；T014 通过后设计 | 设计方 | 排队 | T014；训练前用户确认 |

...
| [T026](assignments/T026-political-special.md) | 政治专项：luna 对全部 340,883 个词做政治预筛（事件、人物、组织、领导人，带“是否常作他义”），复查一遍；按专项规则在 Mac 上重出六份标签到 `labels_political_v1/`（代码见 round6/political_screen_v1/） | 执行方 | v1.1 停在第 9 步：原文核对里 prefix_v2 有 37% 命中的不是政治敏感词，领导人词是 70%。原因：短词被子串匹配、泛指历史和年份、外国组织、不指向人的粗俗语、门槛过宽 → v1.2 加一遍最终核对（只留特指的中国政治敏感内容）、拉丁字母按整词匹配、两字词和数字只在政治文本里生效、门槛只看中国政治类红线。标签改出到 `labels_political_v2/` | T012 ✓、T021 ✓、T024 ✓ |
...
2. **是否进入第二阶段（训练主干）**。T028 已确认只重训读出头到顶了。设计方的方案：
   - 从现在的模型出发全参训练主干，同时训练回答头和提问头，按红线三类逐 token 标签训练（T026 通过后用政治专项标签）；
   - 不生成新数据；
   - 在 L20 上约 4–5 小时训练，再加约 1 小时评测；
   - 新模型评测通过前不替换线上模型。

   用户批准后派单。
3. **第二阶段要不要先补“英文长思考”的安全数据**。T027 显示：官方集里两边都判 safe 的英文长思考，新头会误截 14–23%。我们的训练数据里几乎没有这类文本。
   - 设计方建议：先按红线口径改写 S5 安全一半的生成指令，生成一批中英文长篇安全思考（T015，原计划就有）。
   - 第二阶段可以先用现有数据跑一版，再加上这批数据跑第二版；也可以等数据齐了再跑。
   - 这和“先训练、后生成数据”的顺序有关，请用户定。
...
- **裁判 v4.3（不急）**：
  ...
  只影响以后重判的数据。
- **第二阶段（训练主干）**：T028 已到顶，等用户批准（见“待用户决定”第 2 条）。训练和评测代码已写好（round6/stage2/，含作业文件），**任务单等批准后再派，执行方不要提交这个作业**。

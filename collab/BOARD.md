...
| [T030](assignments/T030-stage2-v2-alert-leader.md) | 第二阶段 v2：警觉档（软目标）、政治组合、领导人数据、普通用法数据、英文红线数据 | 执行方（兼设计） | 训练中（作业 r3，第一张 L20 整卡，131,795 条训练记录） | T029 ✓、T031 ✓、T032 ✓ |
...
| [T033](assignments/T033-distill-4b-v3.md) | 第二阶段 v3：每个角色加一个通用头，用 Qwen3Guard-Stream-4B 逐 token 蒸馏（KL 0.5 + 类别 CE 0.25），红线头照 v2 训练 | 执行方（兼设计） | 训练中（第二张 L20，演示 PVC）；评测作业已备好，v2 退出后在第一张卡上跑 | T030 数据 ✓ |

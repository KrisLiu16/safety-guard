...
- 目标：按用户 2026-09-24 定的提问侧分档，给提问打红线标签，供 T023 缓存上的用户头重训使用。
  ...
  - 裁判：`round6/redline_v1` 的提问模式（`make_tasks.py --target user`，judge `guard-judge-redline-user-v1`）。红线清单、边界代码、变体写法规则、领导人严格规则和词表预筛，都与回答侧相同。回答侧的裁判提示词逐字不变（单测检查），所以 T021 的兜底和补跑不受影响。
...
- 验收（试跑）：
  ...
  - 人工核对：剔除口径存疑后，标签对 ≥ 90%。
- 需要用户决定：**放量规模**（第 5 步）。按 prefix_v2 导出的条数和试跑实测的每条调用次数估算，会在试跑反馈里给出；预计约 4 万条提问、约 5 万次 DeepSeek 调用。
- 反馈里必须报告：
  - 单测；

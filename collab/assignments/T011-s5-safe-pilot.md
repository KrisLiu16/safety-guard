...
- 版本：v2（修订：新增第 6b 步，人工核对 luna 判为不安全的全部条目，验收改用核对后的比例。已完成的步骤不用重做，只补做第 6b 步并更新反馈）
...
- 验收：
  ...
  - 复核：人工核对后，`risk_reasoning_long` 的整段和前缀真正不安全的比例不高于 5%（这一类最容易夹带有害细节，是重点）。luna 的原始 safe 比例单独报告，不作为验收线；

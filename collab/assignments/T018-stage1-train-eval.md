...
- 版本：v3.1（改动只有第 2 步：个别记录重新分词对不上时，丢掉它的位置是允许的）
...
- 步骤：
  ...
  2. 改标签完成后查看 `/work/output/round6/stage1_labels3_v2/report.json`：每个来源的 `kept` 和各项 `dropped_*`。
     - `dropped_offsets` 允许来自“冻结 ids 与重新分词不一致”的个别记录，总数不超过该来源位置数的 0.01%。
     - 超过这个比例，或者 Run A 出现不为 0 的情况，就停下报告。
     - （2026-09-25 设计方决定：v3 第一次运行里 prefix_v2 train 的 25 个位置来自 1 条 safe 记录，第 22510 条，可以接受；请按原 yaml 重新提交并跑完评测。）
...
- 验收：三步 exit code 都是 0；`dropped_offsets` 符合第 2 步；评测 `integrity_pass=true`、`max_prob_diff < 0.001`；官方集的 unique 序列数为 1872。
- 需要用户决定：无（训练已批准）。

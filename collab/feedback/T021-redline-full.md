...
- 对应任务单版本：v3
- 状态：**完成**。标签验收通过，01:03 拷到了 PVC；4 个数据集在平台的数据集服务恢复后于 01:47 删除。Run A 第一阶段的 `--source` 换成了 `trainable.jsonl`，原因见“第 6 步：标签（Run A 第一阶段）”。
...
- **4 个数据集删除失败**：full_runA_stage1、full_runA_dev、full_pv2、full_runA_stage1_fb。
  ...
  - 用户确认平台恢复后，01:47 重新删除，4 个都成功，回执在 `dataset_rm.json`。

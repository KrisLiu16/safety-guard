...
STREAM_PROBABILITY_TOLERANCE = 1e-5
RISK_LABELS = ("safe", "unsafe", "controversial")
ROLES = ("user", "assistant")
FORBIDDEN_ROOTS = ("/work/models", "/work/input", "/work/window", "/work/output/qwen35_full",
                   '/work/round5/data', '/work/output/round5/prefix_v2')


def parse_args(argv=None):
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--bundle", type=Path, default=Path("/work/output/round5/validation/bundle"))
    parser.add_argument("--manifest", type=Path, default=Path("/work/round5/runtime/MODEL_MANIFEST.json"))
    parser.add_argument("--output", type=Path, default=Path("/work/output/round5/validation/bundle_audit.json"))
...
def probabilities(torch, model, ids, runtime=None):
    if runtime is not None:
        values = runtime.classify_prefixes(ids)['probabilities_by_role_all_tokens']
        return {role: dict(zip(RISK_LABELS, values[role][-1])) for role in ROLES}
...
def old_reference(args, torch):
    ...
    # The old loader inserts /work/round4 ahead of this directory. Never let its
    # same-named prototype graph_stream substitute for the new canonical code.
    local = Path(__file__).resolve().parent
    spec = importlib.util.spec_from_file_location('_canonical_core_audit_sources', local / 'test_canonical_l20.py')
    source_helper = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(source_helper)
    runtime_sources = source_helper.load_runtime_modules(local)
    from canonical_text_runtime import CanonicalTextRuntime
    runtime = CanonicalTextRuntime(model, tokenizer, inference_engine='eager',
                                   pad_token_id=selection['execution_contract']['pad_token_id'])
    result = {"selection": selection, "selection_file_sha256": sha_file(args.manifest),
              ...
              "cases": [], "generated_tokens": 0, **runtime_sources}
    for case in examples(tokenizer):
        ...
        canonical = probabilities(torch, model, ids, runtime)
        bulk = probabilities(torch, model, ids)
        result["cases"].append({**case, "canonical_token_ids": ids, "native_tokens": len(ids),
                                ...
                                "risk_probabilities_by_role": canonical,
                                'old_bulk_diagnostic_probabilities_by_role': bulk,
                                'bulk_vs_canonical_diagnostic_max_error': max(max_error(canonical[role], bulk[role]) for role in ROLES)})
    runtime.close()
    if source_helper.runtime_source_receipt(local) != runtime_sources:
        raise RuntimeError('Parent canonical reference used changed runtime sources')
    del runtime
    del model, tokenizer
    gc.collect()
...
def check_threshold(selection, result, language, role, streaming):
    ...
    threshold = selection["thresholds"][mode].get(f"{language}/{role}") if language else None
    ...
    if threshold is not None:
        expected_kind = 'selected_checkpoint_' + mode + '_calibration'
    if actual["kind"] != expected_kind or actual.get('mode') != mode or actual.get('comparison') != '>':
...
def exercise_jsonl(guard, tokenizer, selection, reference):
    ...
    for case in reference["cases"]:
        ...
        if not (result["native_tokens"] == result["net_new_tokens"] == result["real_forward_tokens"] == case["native_tokens"]):
            raise AssertionError("Whole-input accounting is incorrect")
        if result['forward_tokens'] != result['real_forward_tokens'] + result['padding_tokens']:
            raise AssertionError('Canonical future padding accounting is incorrect')
    ...
    def checked(request, case):
        ...
        expected_real = len(ids) - result["restore_position"]
        expected_forward = math.ceil(expected_real / 32) * 32
        if (result["native_tokens"] != len(ids)
                ...
                or result['real_forward_tokens'] != expected_real
                or result['padding_tokens'] != expected_forward - expected_real
                or result['aligned_cache_tokens'] != len(ids) // 32 * 32
    ...
    if "en/user" not in selection["thresholds"]['stream']:
    ...
    checked({'op': 'append_text', 'session': 'bpe', 'text': '请只依据已经提供的内容回答。'}, 'interleaved_session_isolation')
    before_ids = list(guard.sessions['bpe']['session'].token_ids)
    try:
        guard.operation({'op': 'append_text', 'session': 'bpe', 'text': ' x' * 8300})
    except ValueError:
        pass
    else:
        raise AssertionError('Over-limit append was not rejected')
    if guard.sessions['bpe']['session'].token_ids != before_ids:
        raise AssertionError('Rejected append mutated the session')
    checked({'op': 'append_text', 'session': 'bpe', 'text': ''}, 'transaction_after_overlimit_rejection')
    ...
    return {"pass": True, "whole_cases": records, "stream_cases": stream_records,
            ...
            "english_user_threshold_absent": all("en/user" not in values for values in selection["thresholds"].values()),
            "english_user_missing_threshold_not_fabricated": True,
            "cross_role_pass": True, "session_release_pass": True, 'session_isolation_pass': True,
            'overlimit_transaction_pass': True, "generated_tokens": 0}
    ...
    try:
        ...
        sys.path[:] = [str(bundle)] + [path for path in sys.path if path and not (
            ...
            any(os.path.realpath(path) == root or os.path.realpath(path).startswith(root + os.sep)
                for root in (*FORBIDDEN_ROOTS, "/work/round4", "/work/validation", '/work/round5')))]
        ...
        guard_args = SimpleNamespace(max_sessions=4, threshold=None, inference_engine='eager')
        ...
        for case in reference["cases"]:
            ...
            observed = probabilities(torch, model, ids, guard.runtime)
        ...
        loaded_sources = {}
        for name in ("standalone_model", "run_guard", "text_stream_runtime", "window_attention",
                     'canonical_block_engine', 'canonical_text_runtime', 'graph_stream'):
        ...
        report.update(status="completed", phase="finished", **{"pass": True},
                      ...
                      tolerance_rationale="0.005 across independent weight loaders using canonical32; 1e-5 for whole/text-append canonical32 on the same runtime. Old bulk outputs are diagnostic only.",
...
def parent_main(args):
    ...
    try:
        ...
        completed = run_process_group(command, child_log, args.timeout_seconds,
                                      env=environment, cwd=args.bundle.resolve())

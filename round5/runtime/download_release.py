...
def inspect_metadata(metadata):
    ...
    if manifest.get('format') != 'round5-prefix-classifier-bundle-v1':
...
def require_same_bundle(existing, expected):
    ...
    identity_keys = ('format', 'candidate', 'variant', 'checkpoint_sha256',
                     'selection_status', 'promoted_from_initial', 'thresholds', 'threshold_comparison', 'graph_validation_passed',
...
def main():
    ...
    parser.add_argument('--output', type=Path, default=root / 'release_round5')
    parser.add_argument('--remote-bundle', default='/work/output/round5/validation/bundle',

...
def main():
    ...
    parser.add_argument('--pre-collection-timeout',type=int,default=3300,help='Bounded post-training audit deadline, below the worker collection wait')
    args=parser.parse_args();ROOT=args.output_root.resolve();POD=args.pod;JOB=args.job
    if not 1 <= args.pre_collection_timeout <= 3300:parser.error('Post-training audit timeout must be 1..3300 seconds')
                ...
                try:
                    ...
                    with (ROOT/'post_training_audit.log').open('wb') as dest:
                        audit=subprocess.run(K+['exec',POD,'--','/work/modern/bin/python',args.pre_collection_script],
                                             stdout=dest,stderr=subprocess.STDOUT,timeout=args.pre_collection_timeout)

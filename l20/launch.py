"""Start the prepared L20 smoke only when Kubernetes reports one free L20 GPU."""
import argparse
import hashlib
import json
...
JOB = 'safety-guard-l20-smoke-20260923'
...
def verify_bundle():
    expected = (HERE / 'bundle.sha256').read_text().split()[0]
    digest = hashlib.sha256()
    with (HERE / 'bundle.tar').open('rb') as stream:
...
def main():
    ...
    run(kubeconfig, 'create', '-f', str(HERE / 'job.yaml'))
    ...
    run(kubeconfig, 'exec', '-i', pod, '-c', 'probe', '--', 'sh', '-c', 'cat > /work/bundle.tar.part', input_file=HERE / 'bundle.tar')
    run(kubeconfig, 'exec', '-i', pod, '-c', 'probe', '--', 'sh', '-c', 'cat > /work/bundle.sha256', input_file=HERE / 'bundle.sha256')

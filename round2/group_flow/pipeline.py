...
PROMPT_VERSION = "guard-mimo-bilingual-v4"
...
不安全条目可表达威胁、骚扰、造假或侵害隐私的意图，但不要输出可执行的伤害步骤、真实联系方式或伪造的具体事实。尤其当词条是真实人物或机构时，绝不能在 harmful_reply 中编写具体犯罪、贪腐或私生活指控；只能写“我会把关于‘原词’的[未经核实的指控]伪装成事实传播”这类可见的风险行为，方括号内保持抽象占位。safe_refusal 可以拒绝这种行为，也不得复述具体指控。安全条目应与风险条目共享词条和部分语境，使分类依赖行为而不是关键词。
...
def request_body(model_name: str, seed: dict) -> dict:
    task = {"task_key": seed["task_key"], "word": seed["word"],
            ...
            "must_pass_before_output": [
                ...
                "If the word is a real person or organization, do not invent specific criminal, corruption, or private-life allegations; unsafe examples must describe the intent to fabricate or spread an unverified claim without stating the claim.",
...
def validate(payload: dict, seed: dict, expected_version: str = PROMPT_VERSION) -> tuple[list[str], list[dict]]:
    ...
    for expected, ex in zip(slot_records(), examples):
        ...
        row.update({"sample_id": f"{seed['task_key']}-{i:02d}", "task_key": seed["task_key"],
                    ...
                    "prompt_version": expected_version, "annotation_origin": "mimo_synthetic_unverified",

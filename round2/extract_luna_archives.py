#!/usr/bin/env python3
"""Download Aster attempt archives and stream Luna results into category JSONL."""
...
from extract_luna_groups import cli,response_text,validate,_v4  # noqa: E402
...
def main():
    ...
    ap.add_argument("--expected-prompt-version",required=True)
                        ...
                        for member in members:
                            ...
                            if isinstance(response,dict):
                                try:
                                    ...
                                    if version.startswith("guard-luna-bilingual-v") and int(version.rsplit("v",1)[1])>=4:
                                        word_errors,rows=_v4.validate(payload,seed,expected_version=version)
                                    else:
                                        word_errors,rows=validate(payload,seed,expected_version=version)
                            ...
                            full=len(kept)==10 and not word_errors
                            complete+=int(full)
                            original=value.get("records",[])
                            original_full=len(original)==10 and not value.get("errors",[])

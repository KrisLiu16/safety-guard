#!/usr/bin/env python3
"""Make v9 source shards of up to 1,000 terms, 10 terms per request."""
...
INDEX = ROOT.parent / "full-source-index"
...
def main() -> None:
        ...
        with path.open() as source:
            ...
            while True:
                ...
                instruction = {"group_key": task_key, "source_group": group,
                               ...
                               "batch_size": 10, "examples_per_term": 2,
                ...
                shards.append({"task_key": task_key, "source_group": group,
                               ...
                               "model_requests": math.ceil(len(terms) / 10)})
                offset += len(terms)
                shard_no += 1
    ...
    result = {"source_index_sha256": hashlib.sha256((INDEX / "manifest.json").read_bytes()).hexdigest(),
              ...
              "shards": shards,
              "planned_model_requests": sum(x["model_requests"] for x in shards),

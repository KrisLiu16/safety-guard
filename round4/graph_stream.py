"""Experimental W512/batch-one/chunk-eight CUDA Graph classifier runner.
...
independent cache. Unfilled windows, full/memory models, padding, other chunk
sizes and dynamic RoPE are deliberately unsupported.
"""
...
def validate_parameters(window=WINDOW, batch_size=1, chunk_tokens=CHUNK):
    for name, value, required in (("window", window, WINDOW), ("batch_size", batch_size, 1),
                                  ("chunk_tokens", chunk_tokens, CHUNK)):
...
def validate_ids(ids, vocab_size):
    if not isinstance(ids, (list, tuple)) or len(ids) != CHUNK:
        raise ValueError("Graph prototype requires exactly eight token IDs")
...
def validate_cache(cache, torch):
    ...
    if (not values or any(isinstance(v, bool) or not isinstance(v, int) for v in values)
            or len(set(values)) != 1 or not WINDOW <= values[0] <= MAX_TOKENS - CHUNK):
        raise ValueError("Cache must contain one fully initialized prefix of 512..8184 tokens")
    attention = set()
    linear = 0
    for index, layer in enumerate(cache.layers):
        if hasattr(layer, "keys"):
...
class WindowGraphRunner:
    ...
    def __init__(self, model, seed_cache, warmup_ids, *, window=WINDOW, batch_size=1, chunk_tokens=CHUNK):
        ...
        validate_ids(warmup_ids, self._vocab_size)
        validate_cache(seed_cache, self._torch)
        torch = self._torch
        ...
        try:
            with torch.inference_mode():
                ...
                self._ids = torch.empty((1, CHUNK), dtype=torch.long, device="cuda")
                self._positions = torch.empty((1, CHUNK), dtype=torch.long, device="cuda")
                self._offsets = torch.arange(CHUNK, dtype=torch.long, device="cuda")[None, :]
    ...
    def _load(self, cache, ids):
        seen = validate_cache(cache, self._torch)
    ...
    def step(self, ids, cache):
        with self._lock:
            ...
            validate_ids(ids, self._vocab_size)
            validate_cache(cache, self._torch)
            torch = self._torch
            try:
                with torch.inference_mode():
                    ...
                    self._arena.seen = {index: seen + CHUNK for index in self._arena.seen}

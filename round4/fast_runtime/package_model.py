"""Package a fixed Round4 research candidate without reading base model weights.
...
from standalone_model import INFERENCE_ENGINE, runtime_versions, sha256_file, validate_selection
...
def inspect_inputs(args):
    ...
    for name in ("standalone_model.py", "run_guard.py", "text_stream_runtime.py",
                 "graph_stream.py", "graph_text_runtime.py"):
...
def main(argv=None):
    ...
    try:
        ...
        bundle_manifest = {
            "format": "round4-direct-classifier-bundle-v1",
            "created_at": datetime.now(timezone.utc).isoformat(),
            "candidate": selection["candidate"], "variant": selection["variant"],
            "inference_engine": INFERENCE_ENGINE,
            "text_engine_contract": {
                "default": INFERENCE_ENGINE, "explicit_control": "eager", "whole_input": "eager",
                "captured_chunk_lengths": list(range(1, 33)), "window": 512, "batch_size": 1,
                "graph_dispatch": "filled window and exact input chunk length 1..32",
                "eager_dispatch": "unfilled window or large prefill",

...
def parse_args(argv=None):
    ...
    parser.add_argument("--manifest", type=Path, default=Path("/work/round4/fast_runtime/MODEL_MANIFEST.json"))
    parser.add_argument("--output", type=Path, default=Path("/work/output/fast_runtime/bundle_audit.json"))
...
def old_reference(args, torch):
    ...
    if (selection.get("candidate") != "window" or selection.get("variant") != "window"
            or selection.get("inference_engine") != "window_cuda_graph"
            or selection.get("checkpoint_sha256") != "bb16a3a6f87748ce302d6db125822b31add9a7d5210cd44804d9416be44f30d2"):
        raise RuntimeError("The fast bundle audit requires the fixed W512 SFT checkpoint")
...
def exercise_jsonl(guard, tokenizer, selection, reference):
    ...
    for case in reference["cases"]:
        ...
        if (execution["inference_engine"] != "eager_whole" or execution["graph_calls"] != 0
                or execution["graph_forward_tokens"] != 0 or execution["eager_calls"] != 1
                or execution["eager_forward_tokens"] != case["native_tokens"]
                or execution["initialization_included"] is not False):
            raise AssertionError("Whole-input execution or initialization accounting is incorrect")
    ...
    def checked(request, case):
        ...
        execution = result["engine_execution"]
        if (execution["inference_engine"] != "window_cuda_graph"
                or execution["initialization_included"] is not False
                or execution["graph_calls"] + execution["eager_calls"] != result["forward_calls"]
                or execution["graph_forward_tokens"] + execution["eager_forward_tokens"] != result["forward_tokens"]):
            raise AssertionError("Graph/eager dispatch counts do not match actual request forwards")
        ...
        stream_records.append({"case": case, "whole_stream_probability_error": error,
                               **{key: result[key] for key in (
                                   ...
                                   "threshold", "target_binary_decision", "generated_tokens", "engine_execution")}})
        return result
    ...
    execution_totals = {key: sum(row["engine_execution"][key] for row in stream_records) for key in
                        ("graph_calls", "graph_forward_tokens", "eager_calls", "eager_forward_tokens")}
    if execution_totals["graph_calls"] < 1 or execution_totals["eager_calls"] < 1:
        raise AssertionError("Actual text inputs must exercise both graph replay and normal eager prefill")
    observed_stats = guard.runtime.stats()
    if any(observed_stats[key] != value for key, value in execution_totals.items()):
        raise AssertionError("Engine counters include initialization or unaccounted request forwards")
    return {"pass": True, "whole_cases": records, "stream_cases": stream_records,
            "engine_execution_totals": execution_totals,
            "actual_graph_text_calls_verified": True, "initialization_not_counted_as_user_input": True,
...
def child_main(args):
    ...
    try:
        ...
        guard_args = SimpleNamespace(chunk_tokens=32, prefill_chunk_tokens=2048, max_sessions=4, threshold=None,
                                     inference_engine="window_cuda_graph")
        guard = JSONLGuard(guard_args, torch, model, tokenizer, metadata)
        report["engine_initialization"] = guard.engine_metadata
        init = report["engine_initialization"]
        if (metadata.get("inference_engine") != "window_cuda_graph"
                or guard.inference_engine != "window_cuda_graph"
                or init.get("captured_chunk_lengths") != list(range(1, 33))
                or init.get("initialization_excluded_from_request_accounting") is not True
                or not math.isfinite(init.get("capture_seconds", float("nan")))
                or init["capture_seconds"] <= 0
                or init.get("auxiliary_input_tokens", 0) <= 0
                or init.get("shared_graph_arena_storage_bytes", 0) <= 0):
            raise AssertionError("The default fast engine did not initialize all exact graph lengths")
        report["request_stats_immediately_after_initialization"] = guard.runtime.stats()
        if any(report["request_stats_immediately_after_initialization"][key] != 0 for key in
               ("graph_calls", "graph_forward_tokens", "eager_calls", "eager_forward_tokens")):
            raise AssertionError("Graph capture/prefill polluted user-request counters")
        ...
        for name in ("standalone_model", "run_guard", "text_stream_runtime", "window_attention",
                     "graph_stream", "graph_text_runtime"):
            module = sys.modules.get(name)
            if module is None:
                raise AssertionError(f"Required fast-runtime module was not loaded: {name}")
